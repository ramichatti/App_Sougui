...
export class AdminConfirmComponent implements OnInit {
  ...
  getPageLabel(url: string): string {
    ...
    if (url.includes('powerbi')) return 'Gestion des Dashboards';
    if (url.includes('email-settings')) return 'Configuration Email';
  ...
  getPageIcon(url: string): string {
    ...
    if (url.includes('email-settings')) return 'email';
    return 'lock';
  }
}
